...
template <class T, isl::enable_if_t<isl::is_move_constructible_v<T> &&
                                        isl::is_move_assignable_v<T>,
                                    bool> = true>
constexpr void swap(T &a,
                    T &b) noexcept(isl::is_nothrow_move_constructible_v<T>
...
template <class T2, size_t N,
          isl::enable_if_t<isl::is_swappable_v<T2>, bool> = true>
constexpr void swap(T2 (&a)[N],
                    T2 (&b)[N]) noexcept(isl::is_nothrow_swappable_v<T2>) {
  std::swap_ranges(a, a + N, b);
}

// move

template <class T> constexpr isl::remove_reference_t<T> &&move(T &&t) noexcept {
...
template <class T, class U> constexpr T exchange(T &obj, U &&new_value) {
...
template <class T, isl::enable_if_t<!isl::is_nothrow_move_constructible_v<T> &&
                                        isl::is_copy_constructible_v<T>,
                                    bool> = true>
const T &move_if_noexcept(T &x) noexcept {
  return x;
}
template <class T> const T &move_if_noexcept(T &x) noexcept {
  return isl::move(x);
}

// as_const

template <class T> constexpr isl::add_const_t<T> &as_const(T &t) noexcept {
  return t;
}

// declval

template <class T> isl::add_rvalue_reference_t<T> declval() noexcept;
...
template <class T, class U,
          isl::enable_if_t<isl::is_arithmetic_v<T> && isl::is_arithmetic_v<U>,
                           bool> = true>
...
constexpr bool cmp_less(T t, U u) noexcept {
  if constexpr (isl::is_signed_v<T> && isl::is_unsigned_v<U>) {
    if (t < 0)
      return true; // signed (< 0) < unsigned = true
  }
  if constexpr (isl::is_signed_v<U> && isl::is_unsigned_v<T>) {
    if (u < 0)
      return false; // unsigned < signed (< 0) = false
  }
  return t < u;
}
...
template <class T1, class T2> struct pair {
  ...
  explicit(
      ...
      !isl::__is_implicit_default_constructible_v<
          second_type>) constexpr pair() requires(isl::
  ...
  explicit(
      ...
      !isl::is_convertible_v<
          const second_type &,
          second_type>) constexpr pair(const T1 &x,
                                       const T2 &
                                           y) requires(isl::
  ...
  explicit(
      ...
      !isl::is_convertible_v<
          U2 &&,
          second_type>) constexpr pair(U1 &&x,
                                       U2 &&
                                           y) requires(isl::
                                                           is_constructible_v<
                                                               first_type,
  ...
  explicit(!isl::is_convertible_v<const U1 &, first_type> ||
  ...
  explicit(
      ...
      !isl::is_convertible_v<
          U2 &&,
          second_type>) constexpr pair(pair<U1, U2> &&
                                           p) requires(isl::
                                                           is_constructible_v<
                                                               first_type,
                                                               U1 &&> &&isl::
  ...
  constexpr pair(std::piecewise_construct_t, isl::tuple<Args1...> first_args,
  ...
  constexpr pair &operator=(const pair &other) requires(
      ...
      !isl::is_copy_assignable_v<second_type>) = delete;
  ...
  constexpr pair &operator=(pair &&other) noexcept(
      isl::is_nothrow_move_assignable_v<T1> &&isl::is_nothrow_move_assignable_v<
          T2>) requires(isl::is_move_assignable_v<first_type>
  ...
  constexpr pair &operator=(pair<U1, U2> &&other) requires(
      isl::is_assignable_v<first_type &, U1>
          &&isl::is_assignable_v<second_type &, U2>) {
    first = isl::forward<U1>(other.first);
    second = isl::forward<U2>(other.second);
  }
  constexpr void
  swap(pair &other) noexcept(isl::is_nothrow_swappable_v<first_type>
...
template <typename X> auto test_pair_type(std::reference_wrapper<X>) -> X &;
auto test_pair_type(...) -> isl::true_type;

template <typename T1, typename V1 = std::decay_t<T1>,
...
template <class T1, class T2> constexpr auto make_pair(T1 &&t, T2 &&u) {
  return isl::pair<detail::pair_type<T1>, detail::pair_type<T2>>(
...
template <class T1, class T2>
constexpr void swap(pair<T1, T2> &x, pair<T1, T2> &y) noexcept(noexcept(
    x.swap(y))) requires(isl::is_swappable_v<T1> &&isl::is_swappable_v<T2>) {
  x.swap(y);
}

// isl::get

template <class T, class U> constexpr T &get(isl::pair<T, U> &p) noexcept {
  return p.first;
}
...
template <class T, class U> constexpr T &get(isl::pair<U, T> &p) noexcept {
  return p.second;
}
